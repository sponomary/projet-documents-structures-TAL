...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="/">
        ...
        <xsl:text> &#xa;</xsl:text>
        <xsl:text> &#xa;</xsl:text>
        <xsl:text> Affichage en texte simple du patron VERBE—PREP—DET—NOM </xsl:text>
        <xsl:text> &#xa;</xsl:text><xsl:text> &#xa;</xsl:text>
        ...
        <xsl:for-each select="element">
            <xsl:if test="(./data[1][contains(text(),'VER')])">
                ...
                <xsl:if test="following-sibling::element[1][./data[1][contains(text(),'PRP')]]">
                    ...
                    <xsl:if test="following-sibling::element[2][./data[1][contains(text(),'DET')]]">
                        <xsl:variable name="p3" select="following-sibling::element[2]/data[3]/text()" />
                        <xsl:if test="following-sibling::element[3][./data[1][contains(text(),'NOM')]]">
